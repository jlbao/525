...
public class CompanyTagsOrganiser {
	public static class Map extends Mapper<LongWritable, Text, Text, Text>{
		...
		public void map(LongWritable key, Text value, Context context) 
			...
			//String str = value.toString().substring(value.toString().indexOf(",") + 1);
			String companyName = value.toString().substring(0, value.toString().indexOf(","));
			String str = value.toString().substring(value.toString().indexOf(",") + 1);
	...
	public static class Combine extends Reducer<Text, Text, Text, Text> {
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			while(iter.hasNext()){
				...
				context.write(key, new Text(ent.getKey() + "," + ent.getValue()));
			}
		}
	}
	...
	public static class Reduce extends Reducer<Text, Text, Text, Text> {
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			for(Text text : values){
				if(map.containsKey(text.toString())){
					map.put(text.toString(), map.get(text.toString()) + Integer.parseInt(text.toString().split(",")[1]));
				}
				else{
					map.put(text.toString(), Integer.parseInt(text.toString().split(",")[1]));
				}
			}
			...
			PriorityQueue<Entry<String, Integer>> queue = new PriorityQueue<Entry<String, Integer>>(1, comp);
			Iterator<Entry<String, Integer>> iter = map.entrySet().iterator();
			while(iter.hasNext()){
	...
	public static void main(String[] args) throws Exception {
		...
		while(count < 11){
			...
			//fs.copyFromLocalFile(new Path("followerTags.txt"), inputPath);
			...
			FSDataInputStream dis = fs.open(new Path(outputPath + "/part-r-00000"));
